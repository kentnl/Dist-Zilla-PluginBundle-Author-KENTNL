  ...
  my ( $stdout, $stderr ) = Capture::Tiny::capture(
    sub {
      $exception = exception {
        ...
        system( $^X , 'Makefile.PL' ) and die "error with Build.PL\n";
        system( $^X , 'make' )    and die "error running $^X Build\n";
        system( $^X , 'make', 'test', '--verbose' ) and die "error running $^X Build test\n";
      };
    }
  );
