...
if ( $^O eq 'MSWin32' ){
  push @skipfiles, [ qr/^.*\/Makefile$/, "Generated Makefile is outside our control on win32" ];
  push @skipfiles, [ qr/^.*\.bat$/ , "Win32 Bat files are uncontrollable, and need \\r codes to work" ];
  push @skipfiles, [ $_ , "Module::Build intermediary file" ] for ( qr{^.*/_build/.*$}, qr{/Build$}, qr{/MYMETA\.yml$} );
}
